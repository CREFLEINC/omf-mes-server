...
export const MANUAL_PERMISSIONS: Readonly<Record<string, readonly string[]>> = {
  ...
  // 외부 식별자 치환 — 계약이 이 자식 자원의 조회에 「근거: M-01-02 §4」라 적었고, 요구서가
  // 「부모의 화면이 곧 이 오퍼레이션의 화면」이라 답했다(`plan-uiux.md:852`). 부모를 소유한
  // 화면 하나만 적는다.
  'PUT /trace/lots/{lotId}/external-identifiers': ['M-01-02'],
  // 긴급 IQC 생략 요청 — 도출표는 `W-01-13` 인데 계약·§U9 의 화면은 `M-01-13`(「긴급 IQC 생략
  // 요청」)이다. 두 표는 «합집합»이라 같은 키를 다시 적어도 도출된 화면이 빠지지 않는다 —
  // 이 한 줄이 없으면 M-01-13 사용자의 주 버튼이 403 으로 죽는다.
  'POST /trace/lots/{lotId}:request-iqc-skip': ['M-01-13'],
  ...
  // `P-05-02` 비가동 실적 입력 — 요구서 §3은 등록·종료만 적어 수정이 도출되지 않았다.
  ...
  // `W-03-10` 처분 판정 처리 — ⭐ 도출 원천이 화면보다 «낡아» 한 줄도 안 도출된 자리다.
  // 그 화면은 DR-008 확정 3-A 로 2026-08-13 «신설»돼 요구서 §3 보다 새것이고, 그 결과
  // 판정 저장이 `P-02-13`(POP · PQC 검사 결과 입력) 하나만 요구해 **정작 그 화면 담당자가
  // 403 을 받는다**(계약 본문이 「W-03-10 「판정 저장」」이라 화면 코드를 직접 적었다).
  // ⚠ 아래 조회 셋은 계약이 403 을 «선언하지 않아» 오늘은 가드가 안 본다
  // (`permission.guard.ts:40`) — 계약이 403 을 더하는 날 이 세 줄이 그대로 답이 된다.
  // // 결정 — 통보 181
