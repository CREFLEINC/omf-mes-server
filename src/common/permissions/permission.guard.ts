...
/**
 * 계약에 묶인 오퍼레이션에 기능 권한을 요구한다. 계약이 **403 을 249 오퍼레이션**에
 * 선언해 두었고, 그 판정이 여기서 난다.
 *
 * ⛔ **계약이 403 을 선언한 자리에서만 본다.** 선언하지 않은 곳에서 403 을 내면 계약과
 * 어긋난다. 계약 실측: 선언 **249** · 미선언 233.
 *
 * ⛔ 그 249 중 등록되지 않은 오퍼레이션은 **통과가 아니라 던짐**이다(공유계약 F-6 —
 * 「판정할 수 없음」을 「통과」로 처리하지 않는다). 지금 **122/249(49%)** 만 도출되는데,
...
export class PermissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    //    누구인가」라 권한을 물을 자리가 아니다). 계약 실측: 선언 249 · 미선언 233.
