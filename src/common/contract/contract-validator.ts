 ...
 *
 * ⚠ 경계가 **양쪽 다 열려 있다**(`-(2 ** 63) < v < 2 ** 63`). int64 자체는
 * `[-2^63, 2^63-1]` 이라 하한이 «닫힌» 구간인데, 여기서 그것을 그대로 쓰면 안 된다 —
 * **Prisma 가 받는 구간이 열린 구간이기 때문**이다. `|v| >= 2 ** 63` 인 수를 넘기면
 * ⌜Expected BigInt, provided Float⌝ 로 던진다(실측 · `-(2**63)` 은 THROW,
 * `-(2**63) + 1024` 는 OK · 상한도 대칭이다).
 *
 * ⛔ 하한을 `>=` 로 닫아 두면 **배정도로 `-(2 ** 63)` 이 되는 유효 int64 513개**가
 * 검증기를 통과해 그대로 500 이 된다 — 이 검사가 없애려던 바로 그 부류다.
 * 상한도 같은 이유로 `2 ** 63 - 1` 이 아니라 `2 ** 63` 미만이다:
 * `Number('9223372036854775807')` 이 `2 ** 63` 으로 올림돼 역시 못 넘긴다.
 * ⇒ 기준은 「int64 인가」가 아니라 **「배정도로 Prisma 에 넘길 수 있는가」**이고,
 * 표현 가능한 양 끝(`-(2 ** 63) + 1024` · `2 ** 63 - 1024`)은 둘 다 통과한다.
 *
