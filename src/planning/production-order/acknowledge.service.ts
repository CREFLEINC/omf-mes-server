...
/**
 * 본문 400 — **다섯 갈래**다. 넷은 계약 description(ⓐ 강행인데 조정을 실었다 ⓑ 이 P/O 의 영향
 * W/O 가 아니다 ⓒ 같은 `workOrderId` 가 두 번 ⓓ 고칠 칸을 하나도 안 담았다)이고, 다섯째는 컬럼
 * 주석 `production.production_order_acknowledgement.reason` 이 준다 — 「강행이면 필요하다 — 값에
 * 따라 갈리는 규칙이라 CHECK 가 아니라 서버가 건다」.
 * 여섯째 자리(마감 W/O)만 코드가 `STATE_LOCKED` 다 — 계약은 침묵하나 아래 `markMismatch` 의
 * 트리거 이유가 조정에도 그대로 걸린다. 선례 `material-issue-request.service.ts:155`.
 */
export function acknowledgeBodyErrors(body: ProductionOrderAcknowledge, impacted: readonly ImpactedWorkOrder[]): ErrorItem[] {
  const errors: ErrorItem[] = [];
  const adjustments = body.workOrderAdjustments ?? [];
  if (body.decisionCode === PROCEED) {
    if (adjustments.length > 0) errors.push(field('workOrderAdjustments', ERROR_CODE.INVALID, '강행은 작업지시 조정을 함께 보내지 않습니다.'));
    if ((body.reason ?? '').trim() === '') errors.push(field('reason', ERROR_CODE.REQUIRED, '강행이면 사유가 필요합니다.'));
  }
  ...
  for (const [index, adjustment] of adjustments.entries()) {
    ...
    if (closed === undefined) errors.push(field(`${at}.workOrderId`, ERROR_CODE.INVALID, '이 생산오더의 영향 작업지시가 아닙니다.'));
    else if (closed) errors.push(field(`${at}.workOrderId`, ERROR_CODE.STATE_LOCKED, '마감된 작업지시입니다.'));
    if (seen.has(adjustment.workOrderId)) errors.push(field(`${at}.workOrderId`, ERROR_CODE.UNIQUE_VIOLATION, '같은 작업지시가 두 번 왔습니다.'));
 ...
 * ⛔ `production_order.status_code`·`version_no` 를 옮기지 않는다 — P/O 상태는 ERP 수신이 옮기고
 *    이 오퍼레이션은 판정만 적는다(`plan-api.md` 830행). If-Match 토큰이 그대로 산다.
...
export class AcknowledgeService {
  ...
  async acknowledge(productionOrderId: number, version: number, body: ProductionOrderAcknowledge, appUserId: number): Promise<void> {
    await this.prisma.$transaction((tx) => this.commit(tx, productionOrderId, version, body, appUserId));
  }

  private async commit(tx: Prisma.TransactionClient, orderId: number, version: number, body: ProductionOrderAcknowledge, appUserId: number): Promise<void> {
    const [locked] = await tx.$queryRaw<{ version_no: number; last_change_received_at: Date | null }[]>(Prisma.sql`
      SELECT version_no, last_change_received_at FROM planning.production_order
       WHERE production_order_id = ${BigInt(orderId)} FOR UPDATE`);
    if (locked === undefined) throw new NotFoundException('없는 생산오더입니다.');
    // 원인이 `user` 가 아니라 `erpSync` 다 — 계약이 「409 가 나면 「ERP 가 다시 보냈다」」로 못 박았다.
    assertUpdated(locked.version_no === version ? 1 : 0, 'erpSync', { code: VERSION_CONFLICT, currentVersion: String(locked.version_no) });
    ...
    const impacted = await tx.work_order.findMany({
      where: { production_plan: { production_order_id: BigInt(orderId) } },
    ...
    await this.upsertAcknowledgement(tx, orderId, receivedAt, body, appUserId);
  }

  /**
   * 계약 `WorkOrder.poMismatch` — 「서버가 두 자리에서 세운다 — ⓐ 관리자가 「기존 유지(강행)」를
   * 고를 때 ⓑ 「변경 반영」을 골랐는데 그 W/O 를 workOrderAdjustments 로 조정하지 않았을 때」.
   * ⛔ 마감 W/O 는 대상에서 뺀다 — `trg_work_order_closed_immutable`(baseline `migration.sql:3009-3011`)
   *    이 마감 행의 **모든** UPDATE 를 예외로 던져, 마감 W/O 하나가 요청 전체를 500 으로 만든다.
   *    취소 W/O 는 그대로 둔다(트리거가 없고 계약이 침묵한다).
   */
  private async markMismatch(tx: Prisma.TransactionClient, impacted: readonly ImpactedWorkOrder[], decisionCode: string, adjustments: readonly WorkOrderAdjustment[]): Promise<void> {
   ...
   * `acknowledgement_type_code` 를 쓰기로 정해지면 그때 UPDATE 로 바뀐다(R-9).
   * `upstream_version`·`integration_message_id`·`details` 는 수신기 몫이라 안 건드린다.
   */
  private async upsertAcknowledgement(tx: Prisma.TransactionClient, orderId: number, receivedAt: Date, body: ProductionOrderAcknowledge, appUserId: number): Promise<void> {
    const key = { production_order_id: BigInt(orderId), acknowledgement_type_code: ACK_TYPE, received_at: receivedAt };
