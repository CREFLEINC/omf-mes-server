...
/**
 * 검사 결과 조회 2건(I-19 PR ②b) + 저장·수정 2건(PR ③). `:confirm`(PR ④)이 이어 붙는다.
 ...
 * ⛔ 403 게이트는 `derived-permissions.ts` 에 세 자리가 이미 있다 — `manual-permissions.ts` 0줄.
 */
...
export class InspectionResultController {
  ...
  /**
   * 수정. If-Match 는 **필수**라 가드가 이미 막았다.
   * ⚠ 계약이 200 에 ETag 를 선언하지 않았는데 `runVersioned` 는 늘 낸다 — I-1 「알려둘 것」
   *   (`PUT …/steps`)과 같은 자리다. 헤더를 더 내리는 것은 호환 완화라 그대로 둔다.
   */
