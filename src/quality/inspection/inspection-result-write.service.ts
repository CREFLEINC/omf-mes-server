...
/**
 * 계약 `InspectionResultUpdate` — required 0 · 프로퍼티 8. ⛔ `statusCode` 가 없다(확정 경로는
 * `POST statusCode=CONFIRMED`·`:confirm` 둘) · ⛔ 회차 사슬 두 칸이 없다(B-10 「번복은 재검이다」).
 * ⚠ `[string,null]` 형이 하나도 없어 «명시 null 로 해제»하는 칸이 0 이다 — `optional()` 의 null 갈래를 안 쓴다.
 */
...
/**
 * 검사 결과 저장·수정(I-19 PR ③). ⛔ `:confirm` 의 부수효과(LOT 품질 축 전이·보류 해제·
...
export class InspectionResultWriteService {
  ...
  /**
   * ⭐ 작성중인 것만 고친다 — 확정본은 **409 `INVALID_STATE`** 다(계약이 이 자리에만 409 를 문자로
   * 적었다 · `:confirm` 의 재확정은 400 `STATE_LOCKED` 라 봉투가 다르다).
   * 버전 대조가 상태 게이트보다 «먼저»다 — 형제 `:release`·`:close` 선례(`work-order-write.service.ts:78`).
   */
  async update(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (current.version_no !== version) {
        assertUpdated(0, 'user', { code: VERSION_CONFLICT, currentVersion: String(current.version_no) });
      }
      if (current.status_code === CONFIRMED) {
        throw new ConflictException('user', '확정된 검사 결과는 고칠 수 없습니다 — 번복은 재검 회차입니다.', { code: INVALID_STATE });
      }
      // ⛔ 수량 합을 강제하지 않는다 — 작성중이라 M-e ⓑ 뒤 DB 도 안 잡는다(§5-2).
      ...
      assertUpdated(updated.count, 'user', { code: VERSION_CONFLICT, currentVersion: String(current.version_no) });
      // ⭐ 치환이다 — 실으면 그 결과의 측정치 전건을 갈아 끼우고, **생략하면 손대지 않는다**(§1-3).
      ...
      const view = await this.reread(tx, BigInt(inspectionResultId));
      return { view, versionNo: current.version_no + 1 };
    });
  }
