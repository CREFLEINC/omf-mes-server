...
/**
 * 조회 2건(I-19 PR ②b) — 목록(재검 사슬)·상세. 집계 3건·`/measurements`는 별도 컨트롤러다(R-18).
 * ⭐ #298 m-1 — `calibrationExpired` 를 «조용히 무시»하던 자리를 PR ⑤ 가 갚는다. 판정
 * (`calibration.ts`)이 서고 나서야 필터를 여는 순서였다.
 */
...
/** 목록·집계 4건이 **같은 필터 축**(§1-2)을 쓴다 — 두 벌로 짜면 조용히 갈린다. */
