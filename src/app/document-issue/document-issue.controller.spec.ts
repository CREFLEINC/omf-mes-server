...
describe('발행 POST 컨트롤러 배선 (I-27 C3c)', () => {
  it('201 context와 같은 tx를 writer에 전달한다', async () => {
    ...
    await expect(setup.controller.create(request(), body())).resolves.toEqual(setup.response);
...
function controller(options: SetupOptions = {}) {
  ...
  const idempotency = {
    run: jest.fn(async (_context, work) => {
      attempts += 1;
      if (options.firstPathChange && attempts === 1) throw new DocumentIssueTargetPathChanged();
      return {
