...
describe('PurchaseOrderQueryService', () => {
  describe('list', () => {
    ...
    it('목록 — openOnly 를 안 주면 라인 조건을 안 건다', async () => {
      const { prisma, calls } = listStub();
      await new PurchaseOrderQueryService(prisma).list({});
      expect(calls.where).not.toHaveProperty('AND');
    });

    it('목록 — itemId 는 라인에 그 품목이 있는 P/O 만 준다', async () => {
      const { prisma, calls } = listStub();
      await new PurchaseOrderQueryService(prisma).list({ itemId: 42 });
    ...
    it('목록 — itemId 와 openOnly 를 같이 주면 둘 다 건다', async () => {
      const { prisma, calls } = listStub();
      await new PurchaseOrderQueryService(prisma).list({ itemId: 42, openOnly: true });
    ...
    it('목록 — orderDateFrom/To 는 날짜 그대로 비교한다(타임존 캐스팅 없음)', async () => {
      const { prisma, calls } = listStub();
      await new PurchaseOrderQueryService(prisma).list({ orderDateFrom: '2026-08-01', orderDateTo: '2026-08-31' });
    ...
    it('목록 — 기간 없이도 조회된다(기간 필수가 아니다)', async () => {
      ...
      const result = await new PurchaseOrderQueryService(prisma).list({});
    ...
    it('목록 — q 는 MES 발주번호만 검색한다(ERP 번호는 안 본다)', async () => {
      const { prisma, calls } = listStub();
      await new PurchaseOrderQueryService(prisma).list({ q: 'PO-2026' });
  ...
  describe('get', () => {
    it('상세 — 헤더와 라인을 함께 준다(lineNo 오름차순)', async () => {
      ...
      const { detail, versionNo } = await new PurchaseOrderQueryService(prisma).get(1);
  ...
  describe('매퍼', () => {
    ...
    it('매퍼 — 비어 있는 erpPurchaseOrderNo·approvalRequestId 는 널이다(키를 생략하지 않는다)', async () => {
      ...
      const { detail } = await new PurchaseOrderQueryService(prisma).get(1);
