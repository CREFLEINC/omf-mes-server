...
export type ReservationResponse = PagedResponse<InventoryReservationView>;
 ...
 * ⚠ **오늘 언제나 빈 목록이다** — 예약을 «거는» 오퍼레이션이 계약에 없다(§5 · 045 · 첫
 *   사용처는 I-22). ⛔ 잔액 축의 곁에 둔다 — 오퍼레이션 하나에 디렉터리를 만들지 않는다
 *   (I-7 R-13 · §8-4). ⛔ `statusCode` 는 문자 그대로 건다 — 값 목록을 세우지 않는
 *   칸이라(`x-no-code-key`) 대조를 걸면 값이 생길 때 조회가 400 을 낸다.
 */
@Injectable()
...
/**
 * ⭐ `openOnly` 가 이 목록을 `$queryRaw` 로 만든 이유다 — 「아직 소진되지 않은 예약」은 상태
 * 문자열이 아니라 **수량 축**(`reserved − released − consumed > 0`)이고 칸끼리의 뺄셈은
 * Prisma `where` 로 쓸 수 없다(§8-4). 값은 전부 파라미터로 나간다. */
export function reservationWhere(query: ReservationQuery): Prisma.Sql {
  ...
  const id = (column: string, value: unknown): void => {
    ...
    if (parsed !== undefined) conditions.push(Prisma.sql`${Prisma.raw(column)} = ${BigInt(parsed)}`);
  };
  ...
  if (query.statusCode !== undefined) conditions.push(Prisma.sql`status_code = ${query.statusCode}`);
