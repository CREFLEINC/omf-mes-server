...
function stub(statusCode = "RELEASED") {
  ...
  const tx = {
    $queryRaw: () =>
      Promise.resolve([
        { status_code: statusCode, released_at: new Date(), version_no: 1 },
      ]),
    work_order: {
...
describe("W/O 중단·재개 (I-6 PR ④)", () => {
  it("중단 — 사유 코드를 코드값 표와 대조하지 않는다(그룹이 비어 있다 — 문의 035)", async () => {
    ...
    // 사유·발생시각·비고를 담을 칸이 없다 — 상태·버전·수정자만 쓴다(세션에도 손대지 않는다).
    expect(updated).toEqual([
      { status_code: "SUSPENDED", version_no: { increment: 1 }, updated_by: 7 },
    ]);

    // 공백만은 막는다 — 계약이 required 로 적은 칸이고 형식은 우리가 본다.
    await expect(
      service.hold(WORK_ORDER, 1, {
        reasonCode: "  ",
        occurredAt: "2026-09-06T01:00:00.000Z",
      }),
    ).rejects.toMatchObject({
      errors: [{ field: "reasonCode", code: ERROR_CODE.REQUIRED }],
    });
  });
