...
import { currentTerminalMobileProductionWorkerId } from '../auth/terminal-mobile-production-scope';
import { currentSession } from '../auth/session-resolver.service';
import { currentTerminal } from '../auth/terminal-context';

export type MobileProductionWriteActor =
  | { appUserId: number; terminalAudit?: never }
  | { appUserId?: never; terminalAudit: TerminalWorkerAuditActor };

export function mobileProductionWriteActorOf(request: Request, operationKey: string): MobileProductionWriteActor {
  const terminal = currentTerminal(request);
  if (terminal !== undefined) {
    const workerId = currentTerminalMobileProductionWorkerId(request);
    ...
    if (workerId === undefined || typeof workerNo !== 'string' || !workerNo.trim()
      ...
      throw new UnauthorizedException('활성 단말 작업자와 멱등 키가 필요합니다.');
