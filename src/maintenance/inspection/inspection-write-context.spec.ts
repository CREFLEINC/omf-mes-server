...
describe("inspection write context", () => {
  it("세션 주체·귀속 사번·201을 지문에 담는다", () => {
    expect(inspectionWriteContext(createRequest())).toMatchObject({
      key: "idem-1",
      appUserId: 17,
      workerNo: "W-017",
      successStatus: 201,
  ...
  it("세션·사번·본문·오퍼레이션이 달라지면 지문도 달라진다", () => {
    const baseline = inspectionWriteContext(createRequest()).fingerprint;
    for (const request of [
      ...
      expect(inspectionWriteContext(request).fingerprint).not.toBe(baseline);
    }
  });
  ...
  it.each([
    ...
    expect(() => inspectionWriteContext(createRequest({ workerNo }))).toThrow(
  ...
  it("50자 사번은 공백까지 원문 그대로 보존한다", () => {
    ...
    expect(inspectionWriteContext(createRequest({ workerNo })).workerNo).toBe(
      workerNo,
    );
  });
  ...
  it("세션이 붙지 않은 요청은 401이다", () => {
    expect(() =>
      inspectionWriteContext(createRequest({ attach: false })),
...
function createRequest(options: RequestOptions = {}): Request {
  ...
  const request = {
    ...
    path: options.path ?? "/maintenance/inspections",
    headers,
