...
/**
 * BOM 소요식 — 사용처가 둘이라 코어로 올렸다(I-8.md §7-3 · R-19). `:release` 의 자동 발행
 * (`src/production/work-order/`)과 `shortage`(`src/logistics/material-issue-request/`)가
 * **한 함수**를 쓴다 — 도메인 간 import 를 만들지 않으려는 것이 이관의 이유다.
 * ⛔ 순수한 것만 산다 — 어느 `bom_component` 를 담을지(`where`)는 도메인 규칙이라 남겼다.
 */
