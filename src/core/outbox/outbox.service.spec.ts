...
function fake(seed: { queued?: boolean } = {}) {
  ...
  const tx = {
    integration_message: {
      findUnique: record('message.findUnique', () =>
        seed.queued === true ? { integration_message_id: QUEUED_ID } : null,
      ),
  ...
  return { tx: tx as unknown as Tx, calls, created, service: new OutboxService() };
}
...
function input(extra: Partial<OutboxEnqueueInput> = {}): OutboxEnqueueInput {
  return {
    ...
    targetId: 501n,
    payload: { header: { workOrderNo: 'WO-20260906-0001' }, sendItems: ['MATERIAL'] },
    ...extra,
  };
}
...
describe('OutboxService', () => {
  ...
  it('아웃박스 — 같은 키가 이미 있으면 던지지 않고 `alreadyQueued:true` 로 되읽는다', async () => {
    const { tx, calls, service } = fake({ queued: true });

    const result = await service.enqueue(tx, input());

    expect(result).toEqual({ integrationMessageId: QUEUED_ID, alreadyQueued: true });
  ...
  it('아웃박스 — 개발품 판정 칸이 없어 전건 적재한다', async () => {
    ...
    const second = await service.enqueue(
      tx,
      input({ messageKey: outboxMessageKey(IF_WO_CLOSE, 'WO-20260906-0002'), targetId: 502n }),
    );

    expect(first.alreadyQueued).toBe(false);
    expect(second).toEqual({ integrationMessageId: NEW_ID + 1n, alreadyQueued: false });
    expect(created).toHaveLength(2);
    // 품목을 보지도 않는다 — `mdm.item` 에 개발품 칸이 없어 분기 자체를 두지 않았다.
    expect(new Set(calls)).toEqual(new Set(['message.findUnique', 'message.create']));
  });
  ...
  it('아웃박스 — 새 행은 `OUTBOUND`·`PENDING` 이고 `retry_count`·`available_at` 을 쓰지 않는다', async () => {
    ...
    expect(result).toEqual({ integrationMessageId: NEW_ID, alreadyQueued: false });
