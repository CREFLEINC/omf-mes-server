...
--
-- `:cancel` 에 requestBody 자체가 없어 취소 «사유 코드»를 아무도 안 보낸다. 계약 전체에 그 칸이
-- 없고 시드에도 물류 취소 사유 코드 그룹이 없다(`WORK_ORDER_CANCEL_REASON` 은 작업지시 축이다).
-- ⇒ 상수를 박으면 나중에 사유 축이 설 때 상수 행과 진짜 행이 섞인다. README §5 ⌜물리와 계약이
--    다르면 물리를 고친다⌝ · I-3 이 `inbound_variance.reason_code` 에서 같은 판정을 냈다(R-9).
