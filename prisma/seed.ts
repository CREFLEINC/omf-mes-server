...
const ROLES = [
  ...
  {
    // ⛔ 앞의 셋이 없으면 아무도 아무 권한을 줄 수 없다 — 로그인하고, 비밀번호를 바꾸고,
    // 권한을 부여하는 최소 집합이다(권한목록.md 「최초 관리자」).
    //
    // 다음 열하나는 파이널 루틴 REG-ALL-01·PLAN-WO-01 에서 «없어서 막혔던» 자리다 —
    // 단말 등록(W-CO-06)과 ERP 생산오더 → 계획·W/O 전개 → 4M·기본 위치 → 확정·배포
    // → 추가 출고요청, 그리고 그 앞에 서야 하는 마스터(공정·라우팅·설비·위치·BOM 매핑).
    // 교육·테스트 서버는 초기화 뒤 이 계정 하나로 화면을 도는데, 권한이 셋뿐이면
    // 단말 생성부터 403 이라 시나리오가 서지 않는다.
    // 마지막 W-CO-08 은 창고 배치도 — 도면 올리기·배치도 저장이 이 권한이다(#652).
    //
    // 그다음 일곱(W-04-01·02·03·04·12 · W-01-13 · W-01-07)은 파이널 루틴 SHIP-FINAL-01
    // (2026-09-17)에서 «없어서 막혔던» 자리다 — 출하작업지시 편성 → OQC 판정 → 출하 처리 →
    // 출하 확정과, 그 결과를 대조하는 재고 현황·물류 문서 진행현황. 열다섯이면 W-04-01
    // 첫 제출부터 403 이라 제품 출하 시나리오가 서지 않는다.
    // ⛔ 운영 배포도 같은 시드를 쓴다 — 고객이 역할을 나누면 W-CO-02 화면에서 조정한다.
    //    `seedRoles` 는 목록에 없는 부여를 지우지 않아 «더한» 것은 살아남지만, 여기 적힌
    //    스물둘은 시드를 다시 돌 때마다 되살아난다 — 고객이 이 중 하나를 떼려면 역할을 나눈다.
    code: 'ROLE_SYS_ADMIN',
    name: '시스템 운영자',
    permissions: [
      'W-CO-01',
