...
**POP 단말의 재시도 동작** — 배포 시 30~60초 단절이 발생합니다(2026-09-14 블루-그린으로 평소 배포는 끊기지 않게 됐고, 첫 전환·proxy 변경 때만 짧게 끊깁니다 — `RELEASE.md` 「다운타임」). 마이그레이션 이력에 `work_session_idempotency_key` 가 있어 재시도 안전성은 고려된 설계로 보이지만, 단말이 실제로 재시도하는지 확인해야 합니다. 재시도가 없으면 그 사이 실적이 유실됩니다.
