...
| (선택) | 제품 출하 | 출하작업지시 편성(OQC 의뢰 자동 생성) → 모바일 제품 피킹 → W-04-03 출하검사 판정 → 출하 처리 → POP 포장·포장 라벨 → **P-04-05 출하 단위 구성·납품 라벨** → 출하 확정. 제품 재고·검사 기준 시드(`mes-scenario-data.sql`)를 함께 넣으면 위 흐름과 같은 DB 에서 돌릴 수 있다 |
...
# (1b · 선택) 제품 출하용 제품 재고 + 제품 OQC 검사 기준 — 넣지 않으면 아래 9번의 «제품 출하»만 못 한다
...
5. **POP P-04-01 포장 실적** — 피킹된 배분을 상자(포장 단위)에 잇는다. 확정하면 **포장 라벨만** 발행된다(납품 라벨은 여기서 안 나온다).
   시드 POP 단말은 `can_complete_work` 가 켜져 있어 이 화면이 열린다.
6. **POP P-04-05 출하 단위 구성** — 상자들의 **포장 라벨을 스캔**해 출하 단위(`SU-…`)로 묶고 마감한다.
   마감하면 **납품 라벨 1장**이 그 단위에 발행된다 — 라벨 번호가 곧 출하 단위 번호다.
   ⭐ 납품 라벨은 출하 단위당 한 장이다. 종전처럼 배분(제품 LOT × 라인)마다 나오지 않는다.
7. **W-04-12 출하 확정** — 확정은 되돌릴 수 없다. 출하 단위 구성과는 무관하다.

⛔ 라벨은 **POP 이 그린다**(서버는 값만 준다). 포장·납품 라벨 모두 서버 렌디션을 부르지 않는다.
