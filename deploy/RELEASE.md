...
| 배포 | 끊김 (2026-09-14 실측) |
|---|---|
| 평소 | 없음 — 개발 서버 16,330건 중 실패 0. 리허설에서 reload 순간 keep-alive 경합으로 드물게 한 건(약 15만 건 중 1) |
| **블루-그린 첫 전환**(옛 단일 api → proxy) | 1초 안 — 리허설 95ms. 개발 서버는 nginx 이미지를 늦게 받아 약 6초였고, 지금은 미리 받는다. 하노이는 옛 api 없이 첫 설치부터 블루-그린이라 해당 없음 |
