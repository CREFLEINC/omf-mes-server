...
이 표는 원본 OpenAPI와 `v0.1.3-next` 서버 동작이 어긋나는 부분 중 클라이언트가 설계팀 확정 전에 안전하게 처리할 수 있는 것만 정리한다. 설계 판단이 필요한 항목은 임의로 확정하지 않는다.

## P0 — 지금 반영

| 영역                  | 현재 서버 동작                                                                                                        | 클라이언트 대응                                                                                                                 | 근거                           |
| --------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| 사용자 등록 비밀번호  | `POST /app/users`가 계정과 자격을 함께 만든다. `password`(선택, 8자 이상)를 보내면 그 값으로 정해지고 강제 변경이 걸리지 않는다. 생략하면 201 응답 `temporaryPassword`에 임시 비밀번호가 «한 번만» 실린다 | 등록 폼에 선택 비밀번호 칸을 둔다. 비우고 등록했으면 응답의 `temporaryPassword`를 그 자리에서 관리자에게 보여 준다 — 다시 받을 길은 `:reset-password`뿐이다. 201 스키마는 `AppUserCreated`다 | 사용자 결정 2026-09-12         |
| 첫 로그인 강제 변경   | `Session.mustChangePassword`가 필수 칸으로 들어왔다. 로그인과 `GET /app/sessions/current` 양쪽에 항상 실린다                                                                                                  | `true`면 다른 화면으로 보내지 말고 `POST /app/users/me:change-password`로 보낸다. 변경 성공(204) 뒤에는 재로그인시키지 않는다 — 서버가 세션을 유지한다                                                        | 사용자 결정 2026-09-12         |
| 세션 인증             | 로그인 성공 시 `omf_session` HttpOnly 쿠키를 발급하며 로그인 외 현재 제공 API 482건은 유효한 쿠키가 없으면 401이다    | 브라우저 `fetch`는 `credentials: "include"`, Axios는 `withCredentials: true`를 사용하고 401 `ErrorResponse`를 공통 처리한다     | 서버 인증 가드·세션 구현 대조  |
...
| 영역                | 현재 서버 동작                                                                                                | 클라이언트 대응                                                                               | 근거                  |
| ------------------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------- |
| 미구현 4건          | 첨부 업로드 2건·첨부 내용 1건·대시보드 요약 1건은 구현하지 않았다                                             | 기능을 노출하지 않거나 “준비 중”으로 처리한다. 엔드포인트를 호출하지 않는다                   | 커버리지 마감 보고 §1 |
| 발행 산출물 렌더링  | `GET /app/document-issues/{documentIssueLogId}/rendition`은 `MATERIAL_LOT_LABEL`만 PNG로 낸다. 다른 유형은 422다 | 자재 LOT 라벨에서만 미리보기·출력을 노출한다. 산출물을 저장하지 않으므로 URL을 캐시하지 않는다 | 구현 코드 대조        |
