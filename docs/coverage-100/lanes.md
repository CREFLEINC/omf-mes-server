...
| 레인 | 대역 |
|:-:|---|
| A | **069 ~ 089** ⛔ **전건 사용 · 남은 번호 0**(2026-09-08 실측). **077** 은 I-20 R-15 가 「근거 없음」으로 비워 둔 자리를 #414(멱등 409 `code`)가 재사용했고, **089** 는 I-21 계획자가 가져갔다(#416 — 원래 예약분이던 레인 C 재등록 전이 코드를 함께 흡수한다. `transitions.ts:217-218` 주석·통보 154·`plan-integration.md:406` 이 전부 그 문서를 가리킨다) |
...
⚠ **2026-09-08 — A 대역에서 그 규칙이 실제로 걸렸다.** 069~089 가 다 찼다 ⇒ 레인 A 가 다음 통보·질의를 내려면 **먼저 사용자에게 알리고 새 대역을 받아야 한다.** ⛔ 150~179 는 A2 몫이라 빌려 쓰지 않는다.
