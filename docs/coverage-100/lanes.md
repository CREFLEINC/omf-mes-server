# 3레인 병렬 운영 — 공유 규칙 (2026-09-07 · ⛔ 2026-09-14 폐기)

> ## ⛔ 폐기 — 2026-09-14
> 레인(A·A2·B·C) 작업은 **완전히 끝났고, 이 문서의 레인 규칙은 전부 폐기**됐다(사용자 지시). 기록으로만 남긴다.
> - **폐기** — §0 레인 배분(§0-1~§0-3) · §1 겹치지 않게 나눠 둔 것(번호 대역 · 마이그 이름 · PR 소유와 브랜치·제목 접두어 · 파일 소유 · 커버리지 숫자) · §2-1 · §3 · §3-1
> - 설계 요청서 번호는 대역 없이 **기존 최댓값 + 1** 이다 — [`../design-inquiries/README.md`](../design-inquiries/README.md)
> - **레인과 무관해 계속 쓰는 절** — §2 병합 절차 · §2-2 워크트리에서 게이트를 돌릴 때 · §4 절대 금지. 이 셋에 남은 「레인」 표현은 당시 배경 설명이다.
...
### 0-2. ⭐ 2026-09-09 — **C 인수분 중 둘을 B 로 넘긴다**(사용자 지시 · B 가 곧 빈다)

레인 C 가 멈춰 8슬라이스를 A·A2 가 인수했는데(#403~#412), A 는 I-22(9건)·I-23(7건)에 묶여 있고
A2 는 I-14 잔여 2 PR + I-16(7건)에 묶여 있다. **B 가 #347(문의 105 대기)만 남아 곧 빈다** ⇒ 자유 상태인
넷 중 **둘을 B 로 옮겼다**(GitHub 라벨 `Lane-A` → `Lane-B`).

| 이슈 | 왜 B 인가 | 착수 |
|---|---|---|
| **#408** I-35 변경 이력·예비품 엑셀 **2건** | ⭐ **선행 0**(이슈 본문이 「다른 슬라이스가 막히면 여기로 온다」라 적어 뒀다) · ⭐⭐ **`spare_part` 는 B 의 도메인**이다(설비·보전 — B 가 `SPARE_PART_REFERRERS`·`maintenance-result-schema` 를 이미 다룬다) · 계약 `mdm-기준정보.json` 은 **A·A2 의 진행 중 슬라이스가 안 쓴다** ⇒ 충돌 0 | **지금** |
| **#406** I-15 실사 **6건** | 자유 상태 중 **가장 큰 덩어리**이고 계획안부터 한 바퀴라 **막 빈 세션에 맞는다** · 선행 **#404 I-14** 는 A2 가 잔여 2 PR(치환·상신 / `:post` 전기) 중이다 | **#404 병합 뒤** |

#### ⛔ B 로 «안» 넘긴 것과 그 이유

| 이슈 | 이유 |
|---|---|
| **#407** I-17 재생재 1건 | 선행은 0 인데 **`logistics.module.ts` 를 A 의 I-22 와 함께 쓴다**(I-22 §7-2 가 이 겹침을 미리 적어 뒀다). I-22 가 ⑤·⑥·⑦a·⑦b 로 그 파일을 계속 건드리는 동안 넘기면 충돌이 난다 ⇒ **I-22 병합 완료 뒤 넘긴다** ⛔ **2026-09-09 취소 — A2 로 갔다**(아래 §0-3) |
| **#411** 설계 요청 정산·전달(C) · **#412** 레인 C 루틴 마감 | 둘 다 **C 슬라이스의 내용을 알아야 쓸 수 있는 마감 문서**다(어느 통보가 어느 자리에서 났는지). 그리고 실질은 **루틴 끝 일괄 전달**이라 A 의 #339 로 흡수하는 쪽이 맞다 |
| **#405** I-16 · **#404** I-14 | **A2 가 진행 중**(`status:in-progress`) |

### 0-3. ⭐ 2026-09-09 — **A2 가 비어 #407·#410 을 받는다**(A 의 배정 안내)

A2 가 인수분(#403 I-13 · #404 I-14 · #405 I-16)을 **전건 마감**해 잡을 이슈가 0이 됐다. A 가 다음을 배정했다.

| 이슈 | 무엇 | 상태 |
|---|---|---|
| **#407** I-17 재생재 등록 1건 | 몸풀이. 선행 0 · 계약 1건 + 작은 마이그(`logistics.recycle_entry` 2칸) | **지금** |
| **#410** I-23 출하·확정·취소 + 재고 재등록 7건 | 계획안 미작성 — **계획 → 3관점 → 구현 한 바퀴**. 마이그 A14·M-f·U-I · PR 4 예정 | 계획부터 지금 |

⛔ **§0-2 가 「#407 은 I-22 뒤 B 로 넘긴다」고 적은 것은 «취소»됐다** — A2 가 먼저 비었다.
⚠ `logistics.module.ts` 겹침은 그대로다(A 의 I-22 PR ⑦b). **추가만 하는 줄이라 충돌이 자명하다.**

**#410 착수 전 정본 넷**(A 의 안내 · 브리프에 그대로 싣는다):
- ⭐ **[[241]] → 그 답이 [[212]]** — 「취소가 예약의 어느 칸을 움직이나」. ⛔ **제약이 막는 것은 `released_qty` 가 아니라 «순서»다**(212 가 실측으로 정정). 취소는 `consumed` 를 내리고 `released` 를 올린다 · **마이그 0**.
- ⭐ **`slices/I-22.md` §11 ① 이 정본이고 초판은 폐기다** — R-2 가 ⓐ→ⓒ 로 뒤집혔다. I-23 은 `consume()` → `post()` 둘만 부르고 `releaseBalances()` 를 새로 안 만든다. ⛔ `SHIPMENT_REQUEST_LINE` 을 재선언하지 말고 `shipment-progress.ts` 에서 `import` 한다.
- **두 코어의 «둘째» 사용처** — ERP 아웃박스(`src/core/outbox/` · 기존 둘에서 규약이 갈렸다)와 `posting.reverse()`(조정에 취소가 0건이라 이 자리로 왔다 · [[132]]).
- **새 디렉터리** `src/logistics/shipment/` — ⛔ `shipment-allocation/` 과 합치지 않는다.

⚠ **병합은 A(통합자)가 한다** — A2 는 PR 을 열고 리뷰까지만(§0-1 그대로).

#### B 가 착수 전에 볼 것

- **#408** — 계약 `mdm-기준정보.json` · `docs/coverage-100/assignment.tsv`(I-35). audit jsonb 규약을 따른다.
- **#406** — 계획안 **미작성**이라 계획 → 3관점 → 구현 한 바퀴다. 이슈 본문의 ⛔ 네 줄(원장 안 씀 · I-14 마이그가 연 라인 축 판정 · 순환 선행 · `TRUNCATE … CASCADE` 함정)이 이미 실측이다.
- ⭐ **번호 대역은 B 것(`270 ~ 299`)을 쓴다** — §1-1. C 대역(120~149)을 이어 쓰지 않는다.

## 1. 겹치지 않게 미리 나눠 둔 것

> ⛔ 폐기(2026-09-14) — §1-1~§1-5 전부. 머리말 참조.

### 1-1. 설계 문의 번호 — 레인마다 대역이 다르다

> ⛔ 폐기(2026-09-14) — 번호는 대역 없이 기존 최댓값 + 1.

`docs/design-inquiries/NNN-제목.md`. **062 까지 파일 실재 · 063~068 은 I-24 가 예약**했다.

| 레인 | 대역 |
|:-:|---|
| A | **069 ~ 089** ⛔ 소진(2026-09-08) → **`180 ~ 209`** ⛔ **이것도 소진**(2026-09-09 · I-21 이 089+180~186·188, I-22 가 190~208, 화면 정정이 209) → ⭐⭐ **셋째 대역 `240 ~ 269`**(2026-09-09 사용자 배정). **077** 은 I-20 R-15 가 「근거 없음」으로 비워 둔 자리를 #414(멱등 409 `code`)가 재사용했고, **089** 는 I-21 계획자가 가져갔다(#416 — 원래 예약분이던 레인 C 재등록 전이 코드를 함께 흡수한다. `transitions.ts:217-218` 주석·통보 154·`plan-integration.md:406` 이 전부 그 문서를 가리킨다) |
| **A2** ⭐ | **150 ~ 179** (2026-09-08 신설 · **11 사용**) → ⭐ **둘째 대역 `210 ~ 239`**(2026-09-09 사용자 배정 · 레인 C 8슬라이스 중 **6(21 오퍼)을 인수**해 첫 대역 잔여 19개로는 모자란다). A 의 `180 ~ 209` 와 같은 기준(30개)이고, 배정 시점 실재 최대 번호는 **189** 라 충돌 0이다 |
| **B** | **090 ~ 119** ⛔ **소진**(2026-09-09 실측 — **30 전수 사용**. 통합자가 재다가 찾았고 **레인 B 는 모르고 있었다**) → ⭐ **둘째 대역 `270 ~ 299`**(2026-09-09 사용자 배정) |
| **C** | **120 ~ 149** |
...
⛔ 남의 대역은 빌려 쓰지 않는다. 다음 대역이 필요하면 **300~** 으로 이어 붙인다.

⭐ **2026-09-09 — 배정 현황을 한 표로 못 박는다**(그날 하루에 세 번 배정했고 한 번은 통합자가 «이미 배정된 대역을 다시 제안»할 뻔했다).
...
| **069 ~ 089** | A | ⛔ 소진 |
| **090 ~ 119** | B | ⛔ 소진 |
| **120 ~ 149** | C | 18 사용(126~129·136~139 결번) · ⚠ **C 작업 8슬라이스는 A2·A 가 인수**했다 |
| **150 ~ 179** | A2 | 12 사용 |
| **180 ~ 209** | A | ⛔ 소진 |
| **210 ~ 239** | **A2**(둘째) | 배정 2026-09-09 |
| **240 ~ 269** | **A**(셋째) | 배정 2026-09-09 |
| **270 ~ 299** | **B**(둘째) | 배정 2026-09-09 |
| 300 ~ | (미배정) | 다음 요청 시 |

⛔ **새 대역을 제안하기 «전에» 이 표와 `docs/design-inquiries/` 실재 파일을 «둘 다» 봐라.** 표만 보면 발행 안 된 배정을 놓치고, 파일만 보면 예약된 대역을 다시 준다.
...
> ⛔ 폐기(2026-09-14) — 이 절은 레인용 사본이다. 판정 절차는 `README.md` §2 를 본다.
