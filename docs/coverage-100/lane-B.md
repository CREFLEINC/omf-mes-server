...
| 2 | **I-32** 비가동 | 6(완료5·진행1) | I-11(끝남)·I-30 날짜helper(끝남) — **1과 동시 가능** | remarks/최초사번/version 추가3·type완화1, 조건부종료사번1 | µs 준비/물리/조회/쓰기·summary 분리 | P0t+P1a/P1b+P2~P4와 summary 완료, close는 통보109로 재개 |
...
`assignment.tsv`의 I-26·I-27·I-28·I-30·I-31·I-32·I-33 52행을 `src/**/*.ts`의 실제 `@Contract`와 대조했다. 2026-09-09 통보109 결정 뒤 상태는 **구현 49, 진행 가능 1, 기술적 제외 1, 질의 대기 1**이다. `main` `2dbf02a` 시점의 저장소 전체 계약 커버리지는 **453/487**이며, 이 분자에는 다른 레인의 동시 증분이 포함된다.
...
| I-32 | 6 | 5 | 1 | 0 | 0 | [I-32 §13](slices/I-32.md#13-2026-09-09-최종-정산), 통보 109 |
| I-33 | 12 | 12 | 0 | 0 | 0 | [I-33 §13](slices/I-33.md#13-2026-09-09-구현-최종-마감) |
| **합계** | **52** | **49** | **1** | **1** | **1** | close 구현 재개 가능 |
...
- `POST /maintenance/downtimes/{downtimeId}:close`: **통보 109 결정으로 진행 가능**하다. 이 API만 서버 최초 처리시각을 쓰고 오프라인 전송 지연을 허용한다.
...
- 프론트/POP: ETag·멱등 키·사번/단말 귀속과 마이크로초 시각을 계약대로 전달한다. I-26 발번은 질의 해소 전, I-32 close는 서버 구현 완료 전 UI 완료로 표시하지 않는다.
...
- Lane B의 다음 구현은 통보109의 비가동 close 1건이다. 질의 105, 운영 배포 사전점검, 서버 밖 소비자 인수는 별도로 유지한다.
