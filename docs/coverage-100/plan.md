...
| 17 | **I-21** 부적합·처분·특채 | 11 | I-20 | **M-h(항목 2 — I-21 §2-5)** | `transitions.ts` 키 신설 1 · 전이 5(**`C17`·`C18`·`C19` 신설** — I-21 §6-2) · `numbering` `NC` 1줄 | opus · 조회 5 sonnet | ~~3~~ **9**(I-21 §10) | — |
...
| **M-h** | **I-21** | `app.code_value`(`LOT_STATUS_TRANSITION`) · `quality.disposition_decision` | **항목 2**(I-21 §2-5 · 추가만 · 삭제 0 · 백필 0): ⓐ 처분 3전이 코드값 **`C17`·`C18`·`C19` INSERT**(`ON CONFLICT DO NOTHING` + `seed.ts` 동기) — 계약 enum 9값에 `to=SCRAPPED` 인 코드가 0개다(통보 089) ⓑ **`ix_disposition_decision_nonconformance (nonconformance_id)`** — 그 표에 인덱스가 **0개**고 FK 에 자동 인덱스가 없다(**I-20 R-21 과 같은 자리**) |
