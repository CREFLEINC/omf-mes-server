...
| — | **M3 체인 e2e** | | | | | fable | 1 | |
| 19 | **I-22** 출하지시·작업지시·제품 피킹 | 9 | I-8 | A13 | ⭐ **예약 `reserve()` 신설**(2026-09-09 정정 — 43행이 「첫 사용처는 I-22」라 적었는데 이 칸이 `—` 였다. `:pick` 이 예약을 **걸고 곧바로 푼다**(I-22 §0-재수립 R-2 · ⓒ안) · 전용 PR ≤200) | opus | **8** | — |
| 20 | **I-23** 출하·확정·취소 + 재등록 | 7 | I-22·I-5 · **I-19 품질 전이 코어(재등록, A)** | ⭐ `expedited`·`expedite_reason` + `stock_transfer.disposition_decision_id` + 코드값 `C20` (⛔ `confirmed_*`·`cancelled_*` 는 `20260901040000` 이 **이미 세웠다** — 실측 · I-23 §11-1) | ⭐ **넷** — `reservation-qty.ts`(`releaseReservation`) · `transitions.ts`(출하 축·C20) · `outbox.service.ts`(둘째 사용처) · `receipt-posting.ts`(긴급 직행) | opus | **14**(실측 · 예상 4) | ⛔ I-4 와 직렬(이미 끝) · 재등록 전 A 코어 병합·동기화(`lanes.md` §0) |
| — | **M4 체인 e2e** | | | | | fable | 1 | |
...
| 23 | **I-15** 실사 | 6 | I-14 ✅ | `inventory_count_line.counted` 추가 | — | sonnet | **5** | `slices/I-15.md`(레인 B 인수 · 3관점 완료) |
...
| 34 | **I-35** 변경 이력·예비품 엑셀 | 2(구현2·완료) | — | — | — | 조회·업로드 분리 | 2(#548·#550) | `I-35.md` R-1~R-8 · 통보270~271 |
...
| A14·M-f · U-I | I-23 | `logistics.shipment` · `logistics.stock_transfer` | `expedited` · `expedite_reason?` · **`stock_transfer.disposition_decision_id?`**(+FK+부분 인덱스) · `ck_shipment_expedite` ⛔ `confirmed_at`·`confirmed_by` 는 **여기서 세우지 않는다** — `20260901040000` 이 이미 세웠다(I-23 §11-1) |
