...
| U12 | 출고 전표 | 7 | M-01-08 뒤 · W-01-05 · W-01-06 | U11 · U1 | 없음 | 불필요 | ○ 출고 | ○ 출고 | 3 |
...
| U24 | 생산 실적 · 정정 | 5 | P-02-04 · W-02-05 | U23 | 없음 | 불필요 | ○ 생산 | ○ 정정 | 2 |
| U25 | 생산 LOT 완료 · 개체 발번 | 4 | P-02-06 · P-02-05 · W-02-05/06 | U24 | 없음 | 불필요 | — | ○ 수명주기 | 2 |
...
| `PUT /logistics/goods-issues/{goodsIssueId}/lines` | 출고 라인 치환 | — | 멱등, ETag |
| `POST /logistics/goods-issues` | 출고 등록 | M-01-08,W-01-05,W-01-06 | 멱등, ETag, 사번 |
| `POST /logistics/goods-issues/{goodsIssueId}:post` | 출고 전기 | W-01-06 | 멱등, ETag |
...
| 오퍼레이션 | 요약 | 화면 | 헤더 |
|---|---|---|---|
| `GET /logistics/shopfloor-receipts` | 생산창고 입고 목록 | M-01-09,W-06-06 | - |
| `GET /logistics/shopfloor-receipts/{shopfloorReceiptId}` | 생산창고 입고 상세 | M-01-09 | - |
| `POST /logistics/shopfloor-receipts` | 생산창고 입고 확정 | M-01-09 | 멱등, ETag, 사번 |
...
| `POST /production/material-consumptions` | 자재 투입 등록 | P-02-03 | 멱등, ETag, 사번 |
| `GET /production/material-returns` | 자재 반출 목록 | M-02-02 | - |
| `GET /production/material-returns/{materialReturnId}` | 자재 반출 한 건 | M-02-02 | - |
| `POST /production/material-returns` | 자재 반출 등록 | M-02-02 | 멱등, ETag, 사번 |
...
| 오퍼레이션 | 요약 | 화면 | 헤더 |
|---|---|---|---|
| `POST /trace/lots/{lotId}:complete` | 생산 LOT 완료 | P-02-06 | 멱등, ETag, 사번 |
...
| U12 | 기타 출고 결재선 선택 축(`businessUnitId` × `reasonCode` 파생) | 가장자리 — 계약이 「서버가 전표의 reasonCode 로 파생한다」로 이미 못박음 | 0단계에서 끝난다 |
...
| E | **P-02-03** 자재 투입 스캔 | 투입 정정(`:correct`) | 실적에는 `:correct` 가 있는데 **투입에는 없다** | 만들 수 없다. **요청서 후보** |
...
| K | **M-01-09** 생산창고 입고 | 출고·입고 «두 화면이 한 단말»로 합쳐지는 오프라인 시나리오 | 공유계약 §C 가 다루지 않는다(화면 §8) — 큐 순서에 의존하면 `C-2` 건별 되돌리기가 성립 안 함 | U12·U13 을 같은 순서 구간에 둔다. **요청서 후보** |
...
| `POST /production/material-returns` | ⌜⚠ 2026-08-26 — 근거였던 M-02-02(수리 왕복)가 이 경로를 쓰지 않는 것으로 정정됐다 … **부르는 화면이 아직 매핑되지 않았다** — 소유 화면이 정해지면 근거를 다시 적는다⌝ | U23 에 넣어 구현한다. **요청서 후보 — 소유 화면 지정 요청** |
