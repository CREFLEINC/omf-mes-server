...
| U27 | 취급 단위 · 포장 | 7 | P-02-08 · M-04-03 · P-04-01/04 · P-01-02 | U25 | **N-2 — `handling_unit_repack_event(+line)` 신설**(✅ 실측으로 확인 · I-16 재수립 R-1) | ○ | — | ○ 포장 | **5** |
...
| 오퍼레이션 | 요약 | 화면 | 헤더 |
|---|---|---|---|
| `POST /logistics/recycle-entries` | 재생재 등록 | M-01-12 | 멱등, **ETag 없음**(계약 201 에 `headers` 가 0개다 — 파싱 실측 · I-17 §1-1), If-Match 선택(⛔ 안 받는다 — 새 자원이라 대조할 버전이 없다 · C-9), 사번 |
...
| `POST /trace/serial-numbers` | 제품 개체 대량 발번 — 요청 N=개체 N | P-02-04 통합 화면,P-02-12(재사용) | 멱등·사번 필수, If-Match 선택·정상값 비교 없음. 응답 버전 ETag 없음 |
| `GET /trace/lot-lifecycle-events` | LOT 생명주기 변경이력 조회 — 전이 3종 전건 | W-02-05,W-02-06 | - |

I-26: 질의105 회신에 따라 발번 수량은 실적·배분에서 계산하지 않고 요청 N을 N개체로 만든다. 신규 발번→발행은 N개 응답을 단일 targets 요청으로 넘기고 단계별 별도 키를 보존한다. ② 응답 유실은② 같은 키로 원래 회차 재생, 새 발번/재발행이 아니다. GET 첫 쪽으로 성공 배치를 재구성하지 않는다. P-04-04 권한 매핑만으로 시리얼 소비자로 단정하지 않는다. 공용 로그인·CORS와 장기 멱등 보존 한계는 별도 운영 인계다.
...
| **M-04-01** | **1**(⛔ 2026-09-09 정정 — 448행의 배분 목록 오탐을 뺐다) | — | — | — | — | 1 | **U30** |
| **M-04-03** | 3 | — | — | 1 | — | 2 | U27 |
| **M-04-04** | 1 | 1 | — | — | — | — | U10 |
| **M-05-01** | 1 | 1 | — | — | — | — | U33 |
| **M-CO-01** | 1 | 1 | — | — | — | — | U1 |
