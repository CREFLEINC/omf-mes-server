...
| I-16 | 취급 단위·포장·재구성 | 7 | I-12 | **신설 2**(`handling_unit_repack_event(+_line)`) | **⭕ N-2** | **✕ 확정**(원장 미경유) | ⭕ | **5** |
...
| I-23 | 출하·확정·취소 + 재등록 | 7 | I-22·I-5 · **I-19 품질 전이 코어(재등록, A)** | ⭕ 있음(재등록은 `stock_transfer` 재사용 — ⭐ **새 표를 안 세우고 칸 하나를 더한다**: `disposition_decision_id` · I-23 §2-3) | ⚠ 긴급 출하 사유(§I-41) | ⭐ | ⭕ | **14**(실측) |
| I-24 | 생산 계획·생산오더 | 10 | I-6 | 있음 | ⭕(A11 두 칸 — I-24 R-1) | ✕ | ⭕ | 4 |
| I-25 | 공정 인계·수리 왕복 | 6 | I-7 | 있음 | ✕ | ✕ | ✕ | 2 |
| I-26 | 제품 개체 조회·발번 | 2(구현2) | I-7 | 있음 | 현재✕ | ✕ | 묶음 채번 #540 | GET1 + 코어/쓰기 |
...
**원장**: ⛔ 없다(`inventory_transaction` 0건). ⛔ **2026-09-09 정정 — 「예약«만» 움직인다」는 거짓이다**: ⓒ안에서 **`inventory_balance.picked_qty` 도 움직인다**(`available_qty` 는 생성 칸이라 따라 준다). 움직이는 것은 **예약 행 + 잔액 행 둘**이다. 같은 문장의 「I-8 코어 재사용 — **두 번째 사용처**라 코어 판정이 여기서 검증된다」는 **ⓒ안에서 오히려 참이 된다**(`pick()` 의 예약 갈래가 처음 실제로 돈다).
...
**예상 설계 미정**: `shipmentProgressCode` 6값이 2026-09-04 에 신설됐고 `statusCode` 필터는 「칸 불필요」로 닫혔다 — 파생 축이므로 서버가 계산한다. ⛔ **2026-09-09 정정(R-7·R-11) — 「값 이름이 그대로 말한다」는 부족하다**: 이름대로 역순 if 를 쓰면 **배정 0 인 건이 `SHIPPED` 로 나오고 `NOT_ALLOCATED` 가 도달 불가**가 된다(`P=A ∧ S=0` 과 `S=A` 가 `A=0` 에서 공허참). **`A = 0` 을 먼저 잘라 내야** 여섯 값이 다 산다 — 판정식은 I-22 §5-1 이 정본이다.
...
> ⛔⛔ **2026-09-10 폐기 — 아래 문단은 끝났다.** 문의 **089** 는 「질의」가 아니라 «통보»였고,
> 전이 코드는 **`C20`(재등록 → 정상)** 으로 우리가 정했다(통보 218 · 마이그
> `20260910055154_a2_i23_reinstatement_transition_code`). **미룰 것이 0건**이고 I-23 은 7건 전부 섰다.
> 그리고 `from` 은 `['DEFECTIVE']` 하나가 아니라 **셋**이다(`DEFECTIVE`·`NORMAL`·`INSPECTION_PENDING`
> — 반품 갈래 · 통보 184 · #587). 아래는 그때의 기록으로만 남긴다.
...
→ **shipment 가 `goods_issue` 를 만들고 전기한다.** 새 코어를 만들지 않고, «데이터 모양»은 I-4 의 타입을 재사용한다.
⭐ **2026-09-10 근거 교체(I-23 §11-1)** — 판정은 그대로지만 근거가 「2단계 기준 5(새 개념 수)」가 **아니다.**
**「원장 enum 을 지키는 유일한 길」**이다: `InventoryTransaction.sourceDocumentTypeCode` enum 5값에 `SHIPMENT` 가 **없어서**,
전표를 안 만들고 원장만 쓰면 판별자가 어느 값도 못 된다. 전표를 만들면 `GOODS_ISSUE` 로 들어간다.
⛔ 그리고 **`postIssue()` 를 «부른다»** — I-4 가 자기 파일에 적어 둔 금지를 A2 가 실측으로 뒤집었다(I-23 §3-2 ⓓ).
...
**확정**: GET1·POST1 구현. 질의105 회신대로 발번은 실적·양품·배분을 선행조건으로 읽지 않고 요청 N을 개체 N행으로 만든다. serial_component_relation 쓰기0, 상태 전이0, 마이그0.
**체인**: 개체 N행 한 tx → I-27 단일 targets N개. 부분 발번0. 번호 N개는 모든 run tx 밖에서 한 구간으로 예약하고 충돌만 한정 재시도한다. 계정/worker/terminal 지문·bigint문자열·TraceModule AuthModule/NumberingModule 배선을 포함한다.
...
| 8 | **출하가 `GoodsIssueService` 를 부른다.** 도메인 간 service 호출 금지(아키텍처 §1)를 어긴다 | I-23 | shipment 가 `goods_issue` 행을 자기가 만들고 **코어 `postIssue()` 를 부른다**(service 가 아니라 export 함수다 — I-23 §3-2 ⓓ 가 실측으로 정정). ⭐ 근거는 「새 개념 수」가 아니라 **「원장 enum 5값에 `SHIPMENT` 가 없다」**(I-23 §11-1) |
...
- `POST /trace/serial-numbers` `403` — 제품 개체 대량 발번 **구현(회신105·통보104/106/107)**
