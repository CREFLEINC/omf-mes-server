...
|---|---|
| 선행 슬라이스 | S16 |
| 쓰는 표 | `trace.serial_number` — 조회·발번. 조립 관계는 이 두 계약 범위 밖 |
| 마이그레이션 | 없음 |
| posting(원장) 연결 | 없음 |
| 상태기계 | 전이 없음. 생성 초기값 `REGISTERED`를 서비스 상수로 기록(통보104) |
| 구현 분리 | GET #307 + 묶음 채번 코어 #540 + 배치 쓰기 별도 |
| 설계 결정 | 질의105 회신 반영. 번호/If-Match와 귀속·단계별 멱등은 통보106·107 |
...
GET은8개 선택 필터·id ASC·같은WHERE/RepeatableRead total, producedFrom 포함/To 제외를 µs 올림 경계로 보존한다. 저장 상태 원문·nullable producedAt 키 생략, ETag 새 발행0. POST는 요청 N을 개체 N행으로 한 tx에 만들며 생산실적·배분·발행기록을 쓰지 않는다.
...
| posting(원장) 연결 | 없음 (배분은 예약 축). ⛔ **2026-09-09 정정 — 이 줄은 01 자재 피킹의 사실을 04 표에 그대로 옮긴 것이었다**(I-22 계획 §0). **01 자재 피킹**(I-8)은 예약을 **푸는 쪽만** 한다 — 거는 오퍼레이션이 01 계약에 없다(I-8 R-22 · 문의 045). ⭐ **04 제품 피킹(I-22)은 «걸고 푼다»**(⛔ 2026-09-09 **재정정** · I-22 §0-재수립 **R-1** — 앞선 정정이 인용을 잘랐다). **계약 원문**: `:pick` description 은 「서버가 이 피킹의 결과로 `inventory_reservation` 을 **걸고 푼다**(01 자재창고 계약 · `M-01-08` §5-5 와 같은 규약). 자재 피킹의 `:pick` 과 **대칭이다**」이다. 「«건다»」로 끊어 적은 곳은 `GET /logistics/shipment-lot-allocations` 의 `x-internal-note`(08-26 변경 주석) **한 곳뿐**이고, 그 한 곳을 근거로 「거는 쪽만」이라 읽으면 안 된다. ⇒ 한 트랜잭션에서 **걸고 곧바로 푼다**(`balance.picked += Δ` · `reservation.consumed = reserved` · **`reserved` 순변화 0** — ⓒ안 · R-2). 그래도 **예약을 «거는» 코어의 첫 사용처는 I-22** 다(거는 함수가 저장소에 0개였다) — `plan-integration.md:280`·`slices/I-8.md` §3-7·`src/core/inventory-posting/inventory-posting.service.ts:42-43` 이 셋 다 그렇게 예고해 두었고 이 표만 낡아 있었다 |
...
| `transfer-arrive` | `REGISTERED` → `POSTED` | `POST /logistics/stock-transfers/{id}:arrive` | **부른다**(입고 = 2단째). ⛔ ~~부분 도착은 `received_qty` **합**이 담고 전량에서만 `POSTED`~~ — **계약 근거가 0** 이다(전수 확인 · I-13 재수립 R-3). `:arrive` 는 **한 번만** 받는다(재도착 400 `STATE_LOCKED`). 상태를 옮기는 것은 **전량 도착뿐**이고 **부분 도착은 `REGISTERED` 를 유지**한다 — `received_at` 은 차고 잔여는 `IN_TRANSIT` 에 남는다. ⇒ 그 전표를 닫는 길이 계약에 0건이다(문의 124 · 적치 dead end 060 과 같은 형상) |
