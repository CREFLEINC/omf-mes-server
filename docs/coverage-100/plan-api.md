...
| 예상 PR 수 | 3 — ① 조회 GET 7건 ② 마이그 + 입하 등록·수정 + e2e ③ `:split` + 차이(variance) + e2e |
| 설계 미정 자리 · §2 판정 초안 | `:split` 이 원 전표를 어떻게 두는가. 계약이 「정량분과 초과분을 한 트랜잭션으로 등록한다」만 적고 원본 처분을 안 적었다. §2 1단계 = 본길이라 **계약 문자 그대로**(두 건을 새로 만들고 원본은 안 건드림) + 문의. |
