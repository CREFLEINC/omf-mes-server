...
- (I-15) 통보 **272~275**가 정본이다. 개시 스냅샷은 `on_hand_qty`를 `(location,item,lot,uom)`로 합산하고 `IC` 번호·`IN_PROGRESS` 상태로 시작한다(272) · 물리에 `counted boolean`을 추가하고 블라인드 미실사 응답은 장부가 역산되지 않게 마스킹한다. 계약의 `systemQty required`와 생략 설명은 설계 문서 정정 대상이다(273) · 위치 PUT은 빠진 기존 행을 미실사로 되돌리고 새 행의 장부값은 현재 같은 4축 합계로 잡는다(274) · 마감 사유는 완료→상태잠김→미실사→미조정 순서이며 **POSTED 조정 라인만** 완료로 센다(275). 네 건 모두 개발팀 결정 후 통보이며 회신을 기다리지 않는다.
