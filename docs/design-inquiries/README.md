...
## 📨 설계팀에 내는 것 — 세 장이 한 벌이다

| 파일 | 무엇 | 누가 쓰나 |
|---|---|---|
| **[검토요청서-2026-09-10.md](검토요청서-2026-09-10.md)** | ⭐ **맨 처음 여는 한 장.** 답이 필요한 것과 알려 두는 것을 가른다 | 사람 |
| [전달분-2026-루틴마감.md](전달분-2026-루틴마감.md) | 표지 — 채번표 · 시드 · 화면 통지 · 배포 노트 · 안 만든 것 | 사람 |
| [전달분-색인.md](전달분-색인.md) | 요청서 전건 목록(주제별) | ⛔ **자동 생성** |

⛔ 앞의 두 장이 손으로 적는 **수**(요청서 건수 · 질의 건수 · 주제별 건수)는
`python3 scripts/design_inquiries/build_delivery_index.py --check` 가 **색인의 계산과 대조한다.**
요청서를 더하거나 질의가 닫히면 **세 장을 같이 고친다.**
...
### ⛔ 요청서 머리표의 「걸리는 오퍼레이션」 칸에는 **`METHOD /path`** 를 적는다

전달 색인(`scripts/design_inquiries/build_delivery_index.py`)이 **이 칸 하나로 도메인을 읽어**
요청서를 슬라이스별로 묶는다. 슬라이스 이름만 적으면(「I-32 목록·상세」) **기계가 못 읽어 §X
「손질 필요」로 떨어지고, 전달분에 안 실린다.**

```markdown
| 걸리는 오퍼레이션 | I-32 목록·상세 — `GET /maintenance/downtimes` · `GET /maintenance/downtimes/{downtimeId}` |
```

도메인이 **여럿**이면 스스로 「**횡단**」이라 적는다 — 기계는 추측하지 않는다(§7 로 간다).

```markdown
| 걸리는 오퍼레이션 | **횡단** — 발번 `POST /trace/serial-numbers` 과 발행기록 `POST /app/document-issues` 에 같은 규약이 걸린다 |
```

⚠ 2026-09-10 에 B 대역 **일곱 건**(091·096·103·107·108·110·112)이 이 칸에 경로가 없어 §X 에
있었다. 레인 A(통합자)가 각 요청서가 **스스로 적어 둔 슬라이스 이름**에서 고정 계약의 경로를 펴
채웠고, 그 사실을 각 파일에 각주로 남겼다 — **주제를 새로 판정하지는 않았다.**

⭐ 표지(`전달분-2026-루틴마감.md`)가 손으로 적는 두 수(요청서 건수 · 미회신 질의 수)는
`--check` 가 색인의 계산과 **대조한다**. 요청서를 더하거나 질의가 닫히면 표지도 같이 고쳐라.
...
⭐ **번호 — 기존 최댓값 + 1**(2026-09-14 · 레인 작업 완료로 **번호 대역 폐기**). 질의·통보가 같은 번호 축을 쓴다.
옛 대역 안에 남은 결번(244~269 등)은 **메우지 않는다** — 번호가 곧 코드 주석의 참조 키라(`// 결정 — 통보 NNN`) 옛 배정과 섞이면 어디를 가리키는지 흐려진다.
...
> ⛔ **아래 세 문단의 번호 규칙은 폐기됐다**(2026-09-14 · 레인 작업 완료) — 대역 배정 · 「번호는 통합자가 하나씩 준다」 · 「새 대역을 받는다」. 지금 규칙은 위의 **최댓값 + 1** 이다. 어느 번호가 어느 작업에서 났는지의 기록으로만 남긴다.
