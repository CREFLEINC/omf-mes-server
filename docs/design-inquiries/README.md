...
| 120 | 재고 이동에 취소·역분개 경로가 0 이다 | 구현함(I-13 · 미지원 · 취소 enum 3값에 `STOCK_TRANSFER` 없음 · `cancelable:false`) |
| 121 | 창고 간 CHECK 가 완화됐는데 계약·화면은 옛 전제다 | 구현함(I-13 · 서버가 400 `INVALID` 로 대신 막는다 · ⭐ 창고 내 위치 이동은 **흡수처 0**) |
| 122 | 불량 반출 사유를 넣을 길이 0 이다 | 구현함(I-13 · `StockTransferCreate` 에 `reasonCode` 없어 저장 0 · #84 셋째 사용처) |
| 123 | 이동 라인 치환이 도달 불가다 | 구현 예정(I-13 PR ④ · 자물쇠까지만 — 언제나 400 `STATE_LOCKED`) |
| 124 | 부분 도착을 닫는 길이 0 이다 | 구현 예정(I-13 PR ③ · 재도착 거부 · 잔여가 `IN_TRANSIT` 잔류 · 미완 목록에서 사라진다) |
| 125 | 도착이 재고 상태를 무엇으로 되돌리나 | 구현 예정(I-13 PR ③ · 반출 라인의 `from_inventory_status_code` 로 되돌린다 · §2 2-4) |
| 130 | 조정 라인이 잔액 차원 두 칸을 안 싣는데 물리는 NOT NULL 이다 | **답 필요**(I-14 · 대안 둘 = 계약 두 칸 / NOT NULL 완화(053 선례) · I-13 `IN_TRANSIT` 이 등록을 영구 400 으로 막는 사례) |
| 131 | 조정 라인의 실사 원천 칸이 없고 계약 노트 넷이 낡았다 | 구현 예정(I-14 PR ② · 마이그 N-1 로 칸을 연다 · 실사 마감 「조정 완료」를 라인 축으로) |
| 132 | 잘못 전기한 재고 조정을 되돌릴 경로가 0 이다 | 구현함(I-14 · 미지원 확정 · ⇒ `posting.reverse()` 둘째 사용처는 **I-23**) |
| 133 | 조정 헤더에 공장 축이 0 이다 | 구현 예정(I-14 PR ② · 라인 위치에서 역산 · 두 공장이면 400 `INVALID`) |
| 134 | 조정의 ERP 송신 토글이 갈 곳이 없다 | 구현함(I-14 · `sendToErp` 를 받아서 버린다 · `erpMessageQueued` 늘 false) |
| 135 | 조정 채번의 접두어·기간 축이 없다 | 구현 예정(I-14 PR ② · `IA-{YYYYMMDD}-{SEQ4}` · 기간 키는 서버 UTC) |
| 140 | 재포장 이벤트를 담을 표가 계약에 없다 | 구현 예정(I-16 · 「데이터 모델 담당에게 통지」가 세 곳에 살아 있어 **표 둘을 신설**한다) |
| 141 | 취급단위 `status_code` 값 목록이 영영 없는데 그 상수가 409 판정에 쓰인다 | 구현 예정(I-16 · 상수 둘 `OPEN`·`PACKED` · **053 파생이되 판정에 쓰는 첫 자리**) |
| 142 | 확정된 포장을 «여는» 길이 0 이다 | 구현 예정(I-16 · 되돌림·삭제 오퍼레이션 0 · 409 두 뜻 구분 불가 · 고아 `OPEN` HU) |
| 143 | `PUT …/contents` 가 재구성 유형과 여러 원 포장을 못 받는다 | 구현 예정(I-16 · 유형 `RECONFIGURE` 고정 · 빈 배열이 `:pack` 불변식을 사후에 깬다) |
| 144 | `repack-events` 에 페이지·필터가 0 이고 `HU-` 기간 축이 서버 UTC 다 | 구현 예정(I-16 · 전건 반환 · 다른 멱등키 재전송이 빈 이벤트를 남긴다) |
| 145 | 모바일 → POP 2단말 스택이 끊긴다 | 구현 예정(I-16 · HU 가로지르는 이벤트 조회 0 · 하위 포장 조회 축 0 → `P-04-04` 대기 목록 4열 전건 · `P-01-02` 정상 파렛트 차단) |
