...
| **89** | **LOT 상태 전이 코드가 없는 자리 넷**(처분 3 + 재등록 1) · 특채 상태 축 · 후속 롤업 원천 결손 | ⭐ **통보** — 구현 예정(I-21 · `C17`·`C18`·`C19` 신설 + 시드·마이그 3값 · 특채 「유효」 = `APPROVED` · §2 1-1 「정하고 통보」) · ⛔ **`plan-api.md:871`·`I-19.md` §11 ③ 의 「계약이 `SCRAPPED` 전이를 안 적었다」를 정정한다**(계약 `:2460` 이 적었다) · **통보 154 · 회신 대기 12 를 흡수** · §7 재등록 값은 **레인 C 의 판정으로 남긴다** |
