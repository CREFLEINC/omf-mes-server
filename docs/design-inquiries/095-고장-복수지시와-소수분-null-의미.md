...
R5: trigger의 order유일제약을 완화해 한 지시에 여러고장/점검/PM촉발을 보존한다. source전역UNIQUE를 새로 만들지 않는다. I-30 withoutMaintenanceOrder와단수handling ID는 기존직접FK+BREAKDOWN trigger UNION을 함께보고취소도흔적이다. 동일대상 source재선택은115의특정입력422로막되물리N:M을폐기하지않는다. E-O11/12에서 실제신규trigger·다른유형동일숫자ID·중복연결·취소흔적을I-30조회회귀로검증한다.
