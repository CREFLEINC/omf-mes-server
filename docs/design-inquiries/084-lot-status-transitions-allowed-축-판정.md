...
`GET /quality/lot-holds?lotId=…&open=true` 에서 고른다」(`:4410` 인접). 이 오퍼레이션은 애초에
`lotHoldId` 를 입력으로 받지 않으므로 「그 보류 하나를 풀면 몇 건이 남는가」를 구조적으로 알 수 없다.
...
`allowed=true` 인데 실행이 실패/무변화하는 쪽뿐이고, 그건 이 오퍼레이션이 모르는 「어느 보류를
쓸 것인가」에 달려 있다 — 그 정보를 채우는 화면 몫이 `GET /quality/lot-holds` 다(계약이 그렇게
설계했다). 억지로 이 오퍼레이션에서 미리 계산하면 실제로 어떤 보류를 고르느냐에 따라 «틀린»
예측(허위 `allowed=false`)이 나올 수 있어 오히려 더 나쁘다.
