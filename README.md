...
설비 고장 사진 업로드를 사용할 때는 `ATTACHMENT_STORAGE_ROOT`를 컨테이너 안의 절대 경로로 설정하고, 해당 경로를 영속 호스트 디렉터리에 마운트한다. 설정이 없으면 고장 사진 업로드만 503으로 거절한다. 파일은 서버가 생성한 상대 키 아래에 저장하며, 데이터베이스의 SHA-256과 실제 바이트를 대조할 수 있다. 백업에는 데이터베이스와 이 디렉터리를 함께 포함해야 한다.
