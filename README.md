...
첨부 파일(설비 고장 사진 등)은 `ATTACHMENT_STORAGE_ROOT` 아래에 저장한다. 운영 compose는 이 값을 `/var/lib/omf-mes/attachments`로 고정하고 그 자리에 이름 있는 볼륨 `omf-mes_attachments`를 `api-blue`·`api-green`이 함께 건다 — 따로 설정할 것은 없다. 로컬에서 서버를 직접 띄울 때는 `.env`에 호스트의 절대 경로를 넣는다. 설정이 없으면 첨부 업로드만 503으로 거절한다. 파일은 서버가 생성한 상대 키 아래에 저장하며, 데이터베이스의 SHA-256과 실제 바이트를 대조할 수 있다. 백업에는 데이터베이스와 이 볼륨을 함께 포함해야 한다(`deploy/RELEASE.md` 3번).
